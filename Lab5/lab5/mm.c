...
void* mm_malloc(size_t size) {
  ...
  // finding whether free block can accommodate the size of memeory
  reqSize = reqSize + ALIGNMENT;
  ptrFreeBlock = searchFreeList(reqSize);
  // if we cannot find a suitable block, we need to expand the heap
  // printf("I am here \n");
  if(ptrFreeBlock == NULL){
    // if there are no block fits the reqsize than we expand the size of heap
    requestMoreSpace(reqSize);
    ptrFreeBlock = searchFreeList(reqSize);
    precedingBlockUseTag = TAG_PRECEDING_USED;
  }else{
   // if we found the size of block fits reqsize than we label it as used
    precedingBlockUseTag = ptrFreeBlock->sizeAndTags & TAG_PRECEDING_USED;
  }
   // Get the size of the block
  blockSize =  SIZE(ptrFreeBlock->sizeAndTags);
   // Remove the block from he free list
  removeFreeBlock(ptrFreeBlock);

  if((blockSize - reqSize) > MIN_BLOCK_SIZE){
    //if the remain block size is greater than MIN_BLOCK_SIZE, we split it two block 
    ...
    *((size_t*) UNSCALED_POINTER_ADD(ptrFreeBlock, (blockSize - WORD_SIZE))) = blockSize - reqSize ;
    ...
    //if not make a new pointer to the remain size
  ...
  // mark the block we are using now as used  
