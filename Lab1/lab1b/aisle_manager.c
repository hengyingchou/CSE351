...
unsigned short get_section(unsigned long* aisle, int index) {
  // TODO: implement this method
    ...
    return (*aisle) >> (index*16) & SECTION_MASK;  
...
unsigned short get_spaces(unsigned long* aisle, int index) {
  ...
  return ((*aisle) >> (index*16) & SECTION_MASK) & SPACES_MASK; 
...
unsigned short get_id(unsigned long* aisle, int index) {
  // TODO: implement this method
  return (((*aisle) >> (index*16) & SECTION_MASK) & ID_MASK) >> 10;
...
void set_section(unsigned long* aisle, int index,  unsigned short new_section) {
  ...
  unsigned long temp2 = new_section;
  *aisle = (*aisle & ~(temp<<index*16)) | ( temp2 << index*16); 
...
void set_spaces(unsigned long* aisle, int index, unsigned short new_spaces) {
  // TODO: implement this method
  unsigned long temp = 0x03FF;
  unsigned long temp2 =  new_spaces;
...
void set_id(unsigned long* aisle, int index, unsigned short new_id) {
  // TODO: implement this method
  unsigned long temp = 0xFC00;
  unsigned long temp2 =  new_id;
  unsigned long temp3= temp2<<10;
  unsigned long mask = ~(0x3F);

  if( !(temp3&0x03FF) && !(temp2&mask)){
    *aisle = (*aisle & ~(temp<<index*16)) | ( temp3 << index*16); 
...
void toggle_space(unsigned long* aisle, int index, int space_index) {
  // TODO: implement this method
    ...
    int mov = index*16 + space_index;
    static unsigned long temp2 = 0x1; 
    unsigned long temp = (*aisle) & temp2<<mov ;
    (*aisle) = temp > 0 ? (*aisle) & ~(temp2<<mov): (*aisle) | (temp2<<mov);
...
void add_items(unsigned long* aisle, int index, int n) {
  // TODO: implement this method
    ...
    while(i < 10 && n > 0){
      ...
      temp = (*aisle) & temp2<<(mov + i) ;
    
      if(!temp){
        (*aisle) = (*aisle) | (temp2<<(mov + i));
        n--;
      } 
...
void rotate_items_left(unsigned long* aisle, int index, int n) {
  // TODO: implement this method
    ...
    static unsigned long temp2 = 0x1; 
    unsigned long record = ((*aisle) >> (mov))& ((unsigned long)0x03FF);
    (*aisle) = (*aisle) & ~(((unsigned long)0x03FF)<<mov);
    ...
    while(i < n ){
      ...
      temp = record & temp2<<( 10 + i) ;   
      if(temp){
        record = (record | (temp2<<( i)));
      } 
      i++;
    } 

    record = record & (unsigned long)0x03FF;
    (*aisle) = (*aisle) | (record<<mov); 
...
void rotate_items_right(unsigned long* aisle, int index, int n) {
  // TODO: implement this method
    ...
    while(i < n ){
      ...
      temp = record & temp2<<( 10 - n + i) ;
      if(temp){
        record = (record | (temp2<<( 20 - n + i)));
      } 
      i++;

    } 
    record = (record >> 10) & (unsigned long)0x03FF;
    (*aisle) = (*aisle) | (record<<mov); 
